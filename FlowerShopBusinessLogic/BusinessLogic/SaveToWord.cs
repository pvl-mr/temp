...
namespace FlowerShopBusinessLogic.BusinessLogic
{
    static class SaveToWord
    {
        ...
        public static void CreateDocStorePlace(WordInfoStorePlace info)
        {
                ...
                .Create(info.FileName, WordprocessingDocumentType.Document))
            {
                ...
                foreach (var storePlace in info.StorePlaces)
                {
                    ...
                    cellName.Append(new TableCellProperties(
                        new TableCellWidth() { Type = TableWidthUnitValues.Dxa, Width = "2400" }));
                    cellName.Append(new Paragraph(new Run(new Text(storePlace.StorePlaceName))));

                    TableCell cellPerson = new TableCell();
                    cellPerson.Append(new TableCellProperties(
                        new TableCellWidth() { Type = TableWidthUnitValues.Dxa, Width = "2400" }));
                    cellPerson.Append(new Paragraph(new Run(new Text(storePlace.AdministratorName))));

                    TableCell cellDateCreate = new TableCell();
                    cellDateCreate.Append(new TableCellProperties(
                        new TableCellWidth() { Type = TableWidthUnitValues.Dxa, Width = "2400" }));
                    cellDateCreate.Append(new Paragraph(new Run(new Text(storePlace.DateCreate.ToString()))));
